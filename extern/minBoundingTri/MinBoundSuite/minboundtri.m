function [trix,triy] = minboundtri(x,y, thres)
...
id = find(edgelen > thres);
edgelen = edgelen(id);
edges = edges(tags(id),:);
edgeangles = edgeangles(tags(id));
...
for i = 1:length(edgelen)
  ...
  [temp_area, temp_tpoly] = trifun(xy3);
  temp_tpoly = temp_tpoly*rot' + repmat(xy0,4,1);
  
  tempX = temp_tpoly(1:3, 1);
  tempY = temp_tpoly(1:3, 2);
  
%   CVals = checkInsider(basis', [tempX';tempY']);
%   flag_in = ((min(CVals(:)) > 0) & (sum(CVals(:, 1)) <= 1) & (sum(CVals(:, 2)) <= 1) & (sum(CVals(:, 3)) <= 1));

  if (temp_area< area) %&& (flag_in == 1)
    % keep this one
    area = temp_area;
    tpoly = temp_tpoly;
